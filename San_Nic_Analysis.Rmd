...
#Station 2
ggplot(data=urch.mp.2, aes(x=mp, y=red)) +geom_point(aes(col=period),size=4)+geom_segment(aes(xend=c(tail(mp, n=-1), NA), yend=c(tail(red, n=-1), NA)),arrow=arrow(angle=20,type="closed",length=unit(0.2,"cm"))) +scale_color_gradient2(low="red",high="blue",mid="white",midpoint=40) +ggtitle("Density of M. pyrifera vs. S. franciscanus, West End 'Urchin'") + xlab("M. pyrifera density (ind. >1m/m2)") + ylab("S. franciscanus density (ind./m2)")+geom_text(aes(label=period),nudge_x=0.01,nudge_y=0.1,size=3)

#Station 3
ggplot(data=urch.mp.3, aes(x=mp, y=red)) +geom_point(aes(col=period),size=4)+geom_segment(aes(xend=c(tail(mp, n=-1), NA), yend=c(tail(red, n=-1), NA)),arrow=arrow(angle=20,type="closed",length=unit(0.2,"cm"))) +scale_color_gradient2(low="red",high="blue",mid="lightyellow",midpoint=30) +ggtitle("Density of M. pyrifera vs. S. franciscanus, West End 'Kelp'") + xlab("M. pyrifera density (ind. >1m/m2)") + ylab("S. franciscanus density (ind./m2)")+geom_text(aes(label=period),nudge_x = 0.02,nudge_y=0.1,size=3)
...
CA.ggplot <- function(site.dat, scaling=1,choices=c(1,2),plottitle=NULL) { #inputs are a site's dataset, and scaling 1 (default) or 2
  ...
  # The plot
  site.ca.p <-ggplot(spe.ca.sc) +geom_text(data=spe.ca.sc,aes_(x=as.name(axes.names[1]),y=as.name(axes.names[2]),label=as.name(names(spe.ca.sc[4]))),family="serif",col="red")+geom_text(data=sit.sc,aes_(x=as.name(axes.names[1]),y=as.name(axes.names[2]),label=as.name(names(sit.sc[3]))),col="blue") + geom_hline(yintercept=0,linetype=2)+ geom_vline(xintercept=0,linetype=2) +ggtitle(plottitle)
