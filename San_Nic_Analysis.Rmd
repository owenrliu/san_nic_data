...
### Update 5.31.2016: This file has gotten really long, and therefore from now on I am splitting some of these analyses into other files/pieces of code in this project.  Hence there will be some overlap and redundancy.
...
CA.ggplot <- function(site.dat, scaling=1,choices=c(1,2)) { #inputs are a site's dataset, and scaling 1 (default) or 2
  ...
  return(list(plot=site.ca.p,CA=site.ca))
}
...
CA.ggplot(site2)$plot + ggtitle("Correspondence Analysis, West End (Site 2)")

```

PCA with transformation and graphical output

```{r PCA function for benthic data}
compute_PCA <- function(site.dat, scaling=1,choices=c(1,2)) { #inputs are a site's dataset, and scaling 1 (default) or 2
  
  # Transform the data.  Since data is all in density (individuals per m2), we want to apply a Hellinger transform to the rows, but don't need to scale the columns. As a reminder, hellinger transform divides by total site (row) abundance, then square roots.
  
  site.hel <- decostand(site.dat,"hellinger")
  
  # Compute PCA
  site.pca <- vegan::rda(site.hel)
  
  # Extract scores for plotting
  sit.sc <- as.data.frame(scores(site.pca,choices=choices,display="wa",scaling=scaling))
  sit.sc["period"]<-row.names(sit.sc)
  spe.ca.sc <- as.data.frame(scores(site.pca,choices=choices,display="sp",scaling=scaling))
  spe.ca.sc["species"]<-as.numeric(row.names(spe.ca.sc))
  ...
  # The plot
  site.pca.p <-ggplot(spe.ca.sc) +
    geom_text(data=spe.ca.sc,aes_(x=as.name(axes.names[1]),y=as.name(axes.names[2]),label=as.name(names(spe.ca.sc[4]))),family="serif",col="red",size=3)+
    geom_text(data=sit.sc,aes_(x=as.name(axes.names[1]),y=as.name(axes.names[2]),label=as.name(names(sit.sc[3]))),col="blue") +
    geom_hline(yintercept=0,linetype=2)+ geom_vline(xintercept=0,linetype=2)
  
  return(list(plot=site.pca.p,PCA=site.pca))
}
compute_PCA(site2,scaling=1)$plot
compute_PCA(site2,scaling=2)$plot
site2.pca <- compute_PCA(site2,scaling=2)$PCA

# raw PC1, PC2 over time
plot(as.numeric(row.names(site2)),site2.pca$CA$u[,1],type="l")
lines(as.numeric(row.names(site2)),site2.pca$CA$u[,2],col="red",lty=2)

# scaled by eigenvalues
plot(as.numeric(row.names(site2)),scores(site2.pca,display="sites",scaling=1)[,1],type="l")
lines(as.numeric(row.names(site2)),scores(site2.pca,display="sites",scaling=1)[,2],col="red",lty=2)
...
ott.counts.west <- c(rep(0,27),rep(10,3),9,rep(13,4),18,rep(20,3),24,22,20,20,30,29,29,29,29,31,31,18,30,34,42)
ott.counts.north <- c(rep(0,27),rep(4,3),5,rep(1,8),0,1,rep(0,7),1,1,0,1,4,0)
ott.counts.south <- c(rep(0,30),3,rep(0,4),2,rep(0,3),3,6,13,13,2,2,2,2,2,5,5,19,2,8,6)
...
ott.state.bin$ott.count.west <- ott.counts.west
ott.state.bin$ott.count.north <- ott.counts.north
ott.state.bin$ott.count.south <- ott.counts.south
...
ggplot(ott.state.bin,aes(x=ott.count.west,y=red.binary.2)) +
  geom_point(na.rm=T) +
...
site2.expl$ott <- ott.counts.west[-44] # West end site
...
## What about for other sites (with fewer otters?)
## Let's try site 1 (Nav Fac, north side of the island)
#*********************************************************
site1.inverts <- site1[,1:12] # invert columns
names(site1.inverts) <- c("pat","red","purp","wavy","astro","derm","halc","halr","limp","paras","pis","pyc") # rename variables
site1.inverts.norm <- decostand(site1.inverts,method="normalize") # Euclidean norm (chord) transformation (MRT is a euclidean procedure)
# explanatory variables: algaes, period(time), and otter counts. No normalization of these variables. keeping period as a separate
# dataframe/vector, because it is dominant
site1.expl <- site1[,13:19]
site1.period <- data.frame(period=as.numeric(row.names(site1.expl)))
site1.expl$ott <- ott.abun.key[,"north"][match(row.names(site1),ott.abun.key$period)]
names(site1.expl) <- c("cys","lam","ymac","youn","eis","pter","mac","ott")

# The MRT procedure
# ************************
# Just based on time
site1.mvpart.per <- mvpart(data.matrix(site1.inverts.norm)~., site1.period,cp=0,margin=0.08,xv="pick",xvmult=100,legend=F)

## What about constrained by algae and otters?
site1.mvpart.ott <- mvpart(data.matrix(site1.inverts.norm)~., site1.expl,cp=0,margin=0.08,xv="pick",xvmult=500,legend=F)

# Time and all others combined
site1.mvpart.all <- mvpart(data.matrix(site1.inverts.norm)~., cbind(site1.expl,site1.period),cp=0,margin=0.08,xv="pick",xvmult=500,legend=F)


### ... and a southern site (site 4, Dutch Harbor)
#*********************************************************
site4.inverts <- site4[,1:12] # invert columns
names(site4.inverts) <- c("pat","red","purp","wavy","astro","derm","halc","halr","limp","paras","pis","pyc") # rename variables
site4.inverts.norm <- decostand(site4.inverts,method="normalize") # Euclidean norm (chord) transformation (MRT is a euclidean procedure)
# explanatory variables: algaes, period(time), and otter counts. No normalization of these variables. keeping period as a separate
# dataframe/vector, because it is dominant
site4.expl <- site4[,13:19]
site4.period <- data.frame(period=as.numeric(row.names(site4.expl)))
site4.expl$ott <- ott.abun.key[,"north"][match(row.names(site4),ott.abun.key$period)]
names(site4.expl) <- c("cys","lam","ymac","youn","eis","pter","mac","ott")

# The MRT procedure
# ************************
# Just based on time
site4.mvpart.per <- mvpart(data.matrix(site4.inverts.norm)~., site4.period,cp=0,margin=0.08,xv="pick",xvmult=100,legend=F)

## What about constrained by algae and otters?
site4.mvpart.ott <- mvpart(data.matrix(site4.inverts.norm)~., site4.expl,cp=0,margin=0.08,xv="pick",xvmult=500,legend=F)

# Time and all others combined
site4.mvpart.all <- mvpart(data.matrix(site4.inverts.norm)~., cbind(site4.expl,site4.period),cp=0,margin=0.08,xv="pick",xvmult=500,legend=F)
### TONS of error here; doesn't seem like the data group well!  Maybe not enough variation (stationary)
...
Does a redundancy analysis tell us something similar?

```{r RDA, inverts predicted by algaes and otters}
# Again, starting with Site 2 as an example
# Using the hellinger-transformed data, and the algaes and otters as explanatory variables

site2.rda <- rda(site2.inverts.hel~.,site2.expl)
summary(site2.rda, scaling=2)

# Adjusted R^2 of the model
# ************************
(R2 <- RsquareAdj(site2.rda)$r.squared) # original R2
(R2 <- RsquareAdj(site2.rda)$adj.r.squared) # adjusted R2

# Triplots of the result (triplots since they represent sites (i.e. objects/periods), response variables(i.e. inverts) and explanatory variables (i.e. algaes and otters)
# **********************
# Scaling 1: distance triplot (distances between objects represent their Euclidean distance in multidimensional space)
plot(site2.rda, scaling=1, main="Triplot RDA site2.hel ~ site2.expl - scaling 1")
# adding arrows for species

spe.sc <- scores(site2.rda, choices=1:2,scaling=1,display="sp")
arrows(0,0,spe.sc[,1],spe.sc[,2],length=0,lty=1,col="red")

# Scaling 2: correlation triplot(angles between descriptors represent their correlations)
plot(site2.rda, scaling=2, main="Triplot RDA site2.hel ~ site2.expl - scaling 2")
# adding arrows for species

spe2.sc <- scores(site2.rda, choices=1:2,scaling=2,display="sp")
arrows(0,0,spe2.sc[,1],spe2.sc[,2],length=0,lty=1,col="red")


### We can use permutation to test significance of the model and of indiivdual axes
# function is called vegan::anova(), even though it is now the same as a classical anova

# Global test of the RDA
anova.cca(site2.rda,step=1000) # highly significant (Pr>F = 0.001)
# Tests of the canonical axes
anova.cca(site2.rda,by="axis",step=1000) # Only the first two axes are significant at the .05 (and .01) levels
...
# RDA function for any site, starting with raw site data
site.RDA <- function(site.dat,region,constrain=FALSE) {
  # Transform species data
  inverts <- site.dat[,1:12] # invert columns
  names(inverts) <- c("pat","red","purp","wavy","astro","derm","halc","halr","limp","paras","pis","pyc") # rename variables
  inverts.hel <- decostand(inverts,method="hel") # Hellinger transform
  
  # explanatory variables: algaes, period(time), and otter counts. No normalization of these variables.
  
  reg <- data.frame(period=ott.abun.key$period) # temp dataframe for otter counts appropriate to the given site's region
  reg$ott <- switch(region, "west"=ott.abun.key$west,"north"=ott.abun.key$north,"south"=ott.abun.key$south)
  
  expl <- site.dat[,13:19] # algaes
  expl$ott <- reg$ott[reg$period %in% intersect(row.names(site.dat),reg$period)] #otters
  names(expl) <- c("cys","lam","ymac","youn","eis","pter","mac","ott") # change variable names
  
  # Using the hellinger-transformed data, and the algaes and otters as explanatory variables
  if(constrain) {site.rda <- rda(inverts.hel~cys + lam + ymac + youn + eis + pter + mac + Condition(ott),expl)}
  else(site.rda <- rda(inverts.hel~cys + lam + ymac + youn + eis + pter + mac + ott,expl))
  print(summary(site.rda, scaling=2))
  
  # Adjusted R^2 of the model
  # ************************
  print(paste("r squared = ",RsquareAdj(site.rda)$r.squared)) # original R2
  print(paste("adj r squared = ",RsquareAdj(site.rda)$adj.r.squared)) # adjusted R2
  
  # Triplots of the result (triplots since they represent sites (i.e. objects/periods), response variables(i.e. inverts) and explanatory variables (i.e. algaes and otters)
  # **********************
  # Scaling 1: distance triplot (distances between objects represent their Euclidean distance in multidimensional space)
  plot(site.rda, scaling=1, main=c("Triplot RDA inverts.hel ~ expl - scaling 1 - "))
  # adding arrows for species
  
  spe.sc <- scores(site.rda, choices=1:2,scaling=1,display="sp")
  arrows(0,0,spe.sc[,1],spe.sc[,2],length=0,lty=1,col="red")
  
  # Scaling 2: correlation triplot(angles between descriptors represent their correlations)
  plot(site.rda, scaling=2, main=c("Triplot RDA inverts.hel ~ expl - scaling 2 - "))
  # adding arrows for species
  
  spe2.sc <- scores(site.rda, choices=1:2,scaling=2,display="sp")
  arrows(0,0,spe2.sc[,1],spe2.sc[,2],length=0,lty=1,col="red")
  ...
  # Global test of the RDA
  print(anova.cca(site.rda,step=1000)) #
  # Tests of the canonical axes
  print(anova.cca(site.rda,by="margin",step=1000)) #effects of individual explanatory terms
}
```
...
# We'll do a redundancy analysis of fish
