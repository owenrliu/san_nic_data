...
red.mac <- left_join(mac.out,red.out)
red.mac <- mutate(red.mac,upper.red=rho.red+sd.red,lower.red=rho.red-sd.red,upper.mac=rho.mac+sd.mac,lower.mac=rho.mac-sd.mac)

cols <- c("Red causes mac"="red","Mac causes red"="green")
ggplot(red.mac) + geom_ribbon(aes(x=L,ymin=lower.red,ymax=upper.red),fill="red",alpha=0.3)+
  geom_ribbon(aes(x=L,ymin=lower.mac,ymax=upper.mac),fill="green",alpha=0.3)+
  geom_line(aes(x=L,y=rho.red,col="Red causes mac")) +
  geom_line(aes(x=L,y=rho.mac,col="Mac causes red")) +
...
Generic function for CCM plot and numeric output, given a dataset and 2 variables of interest